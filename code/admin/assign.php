...
$sql3 = "SELECT * FROM car where c_stat='2'";
...
$sql2 = "INSERT INTO `tbl_check`(`car_id`, `checker_id`,`stat`) VALUES ('$car','$checker','$stat')";

if($conn->query($sql2) === TRUE){
    $sql22 = "UPDATE `car` SET `c_stat`='3' WHERE `car_id`='$car'";
...
<form action="" method="post">
    <input type="hidden" name="car" value="<?php echo $row1['car_id'];?>">
                                    <select  id="checker" name="checker" class="form-control h-50 w-500" required>
                                    <option value="" hidden>-Select-</option>
                                    <?php
                                    $dis=$row35['district'];
                        $check="SELECT * FROM tbl_checker where district='$dis'";
                        $check_result=$conn->query($check);
                        if($check_result->num_rows > 0){
                            while($m= $check_result->fetch_assoc()){
                                ?>
                                 <option value="<?php echo $m['checker_id'];?>"><?php echo $m['fname'],' ',$m['lname'];?></option>
